(ns internza.middleware
  (:require [internza.session :as session]
            [internza.layout :refer [*servlet-context*]]
...
(defn production-middleware [handler]
  (-> handler
      ...
      (wrap-defaults
        (assoc-in site-defaults [:session :session-store] (memory-store noir-session/mem))) ;;changed :store to :session-store ;;changed from session to noir-session
